...
class App extends Component {
  constructor() {
    ...
    const keys = Object.keys(defaultDimensions).filter(
      k => typeof defaultDimensions[k] === "number"
    );
  ...
  render(props, { inputs }) {
    ...
    return (
      <Form inputs={this.state} updateInputs={this.updateInputs.bind(this)} />
    );
  }
}
...
const Input = ({ name, value, updateInputs }) => {
  return (
    <div className="input">
      <label>{name}</label>
      <input type="number" name={name} value={value} onInput={updateInputs} />
    </div>
  );
};
