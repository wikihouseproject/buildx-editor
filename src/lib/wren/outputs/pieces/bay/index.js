    ...
    sides: {
      inner: {
        leftWall: side(inputs,
          ...
          { x: offset + points.inner.BL[0], y: (points.outer.BL[1] - points.inner.BL[1]), z: bayZ },
          { y: Math.PI/2 }
        ),
        rightWall: side(inputs,
          [points.inner.TR, points.inner.BR],
          { x: offset + points.inner.BR[0] - inputs.materials.plywood.depth, y: (points.outer.BR[1] - points.inner.BR[1]), z: bayZ },
          { y: Math.PI/2 }
        ),
        leftRoof: side(inputs,
          [points.inner.TL, points.inner.T],
          { x: offset + points.inner.TL[0], y: (points.outer.BR[1] - points.inner.BR[1]) + (points.inner.BL[1]-points.inner.TL[1]), z: bayZ },
          { y: Math.PI/2, z: Math.PI }
        ),
        rightRoof: side(inputs,
          [points.inner.TR, points.inner.T],
          { x: offset + points.inner.TR[0], y: (points.outer.BR[1] - points.inner.BR[1]) + (points.inner.BR[1]-points.inner.TR[1]), z: bayZ-bayLength },
          { y: -Math.PI/2, z: Math.PI }
        ),
        floor: side(inputs,
          [points.inner.BL,points.inner.BR],
          { x: offset + points.inner.BR[0], y: (points.outer.BR[1] - points.inner.BR[1]), z: bayZ },
          { y: Math.PI/2 }
        )
      },
      outer: {
        ...
        leftWall: side(inputs,
          ...
          { x: offset + points.outer.BL[0] - inputs.materials.plywood.depth, y: plyThickness, z: bayZ },
          { y: Math.PI/2 }
        ),
        rightWall: side(inputs,
        ...
        leftRoof: side(inputs,
          ...
          { x: offset + points.outer.TL[0], y: (points.outer.BL[1]-points.outer.TL[1]) + plyThickness*2, z: bayZ },
          { y: Math.PI/2, z: Math.PI }
        ),
        rightRoof: side(inputs,
          [points.outer.TR, points.outer.T],
          { x: offset + points.outer.TR[0], y: (points.outer.BR[1]-points.outer.TR[1]) + plyThickness*2, z: bayZ-bayLength },
          { y: -Math.PI/2, z: Math.PI }
        )
      }
    }
