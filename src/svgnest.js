...
function runSvgNest(svgData, binId, options, callback) {
  ...
  function done(err) {
    ...
    if (!lastResult) {
      return callback(
        new Error("Timeout without a single placement"),
        null,
        {}
      );
    }
    ...
    if (details.totalParts != details.placedParts) {
      ...
      return callback(
        new Error(`Not all parts were placed: ${placed}`),
        null,
        details
      );
    }
    ...
    const results = lastResult.svglist.map(function(e) {
      return exportSvg(e);
    });
    if (!(results.length >= 1)) {
      return callback(
        new Error("No sheets where returned from nesting"),
        null,
        details
      );
    }
